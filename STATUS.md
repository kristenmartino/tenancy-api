...
> **Active focus:** V2 highlight (OCR-anchored bbox overlay) **shipped and production-verified** end-to-end ([tenancy-api#26](https://github.com/kristenmartino/tenancy-api/pull/26) + [tenancy#18](https://github.com/kristenmartino/tenancy/pull/18)). Backend emits per-line `bboxes` arrays via pdfplumber alignment; frontend renders multi-rect overlays. Filled fields land pixel-tight, multi-line values render Word-style, blank-template fields anchor on the labeled blank line (not the section header). The highlight saga is closed. Only V0 item remaining: record the 60-90s demo. Case study is the deliverable; productize gates explicitly out of scope.

> **Open question:** none in flight. Highlight direction shipped (OCR-anchored bboxes); Textract `SELECTION_ELEMENT` for checkbox/signature geometry stays deferred (no customer demand yet, and ~not-highlighted is the documented acceptable behavior for those field classes).

## Next 3

1. **[#2]** Record the 60-90s main SaaS demo (`effort-day`) — script ready at the frontend repo's `docs/demo-script.md`. The last open V0 item.
2. **Record the 30s MCP bonus clip + tools screenshot** (`effort-hour`) — script at [`docs/demo-mcp.md`](docs/demo-mcp.md); capture `docs/mcp-tools.png` from Claude Desktop.
3. **Wire the recorded assets into both READMEs** (`effort-hour`) — replace the "forthcoming" pointers in this repo's MCP section and the frontend README, then flip these items closed.
...
- v2 **shipped**: OCR-anchored per-line bboxes via pdfplumber + rapidfuzz. Backend ([tenancy-api#26](https://github.com/kristenmartino/tenancy-api/pull/26)) + frontend ([tenancy#18](https://github.com/kristenmartino/tenancy/pull/18)) both merged and production-verified.
- **[tenancy-api#17]** — v3: AWS Textract `SELECTION_ELEMENT` for checkbox / signature geometry, ~99% positional accuracy on typed text. Promote when a customer asks for checkbox highlights or the v2 ~3-4% drift on filled values becomes a blocker.
...
- **README truth-pass** — a review caught three stale claims, now fixed: the frontend bullet said click-to-highlight used the PDF.js text layer (it's canvas rendering + OCR-anchored bbox overlays); interactive exception resolve was listed as scaffolded/read-only (it shipped — approve/edit/reject wired to the resolve endpoint, edit writes back); and the MCP demo video + tools screenshot were linked as if present (now marked forthcoming). The scaffolded list's deferred item is now checkbox/signature highlight geometry (opt-in Textract path, [PR #27](https://github.com/kristenmartino/tenancy-api/pull/27), not merged or verified against a real lease).
- **Demo-prep polish + writeup pass shipped** (frontend) — responsive layout, not-found + status-aware empty states, confidence tooltips, print stylesheet + sample-abstract.pdf, README refresh on both repos. Only the demo recordings remain of the old Next 3.
- **Bbox overlay v2 production-verified** ([tenancy#18](https://github.com/kristenmartino/tenancy/pull/18), [tenancy-api#26](https://github.com/kristenmartino/tenancy-api/pull/26)) — OCR-anchored per-line bboxes verified end-to-end on fresh lease `45314996` (2026-05-21): filled fields land pixel-tight (`property.street_address` → "at 1621 James Ave Waco, Texas 76706" line at `y≈0.93`), multi-line values render as multi-rect Word-style highlights (`parties[1].address` → 2 stacked rects), blank-template fields anchor on the labeled blank line rather than the section header above (`rent.base_monthly_rent` → 3 rects on the "$_ per _" placeholders in section 6, header untouched). User confirmed live behavior: "everything highlighted is correct. some things don't highlight." Coverage gaps (checkboxes, signatures, hand-fill) fall under the deferred Textract v3 path ([#17](https://github.com/kristenmartino/tenancy-api/issues/17)), not a renderer issue. The 12+ matcher-iteration + LLM-bbox detour saga is closed.
- **Models in use** — extraction: `EXTRACT_MODEL=claude-sonnet-4-6` (env-overridable, currently un-date-pinned — re-pin if drift across minor versions becomes a concern); template detection: `TEMPLATE_MODEL=claude-haiku-4-5-20251001`; Q&A: `QA_MODEL=claude-haiku-4-5-20251001`. All three are env vars in `graph.py` / `qa.py`. Recording here so the bbox accuracy + drift numbers above can be re-tested against the same model snapshot if needed.
